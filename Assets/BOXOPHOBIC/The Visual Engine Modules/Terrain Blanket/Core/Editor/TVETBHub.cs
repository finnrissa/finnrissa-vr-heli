...
{
    public class TVETBHub : EditorWindow
    {
        float GUI_HALF_EDITOR_WIDTH = 220;

        string autorunPath;
        string assetFolder = "Assets/BOXOPHOBIC/The Visual Engine Modules/Terrain Blanket";
        ...
        static TVETBHub window;

        //Vector2 scrollPosition = Vector2.zero;

        [MenuItem("Window/BOXOPHOBIC/The Visual Engine/Hub/Terrain Blanket", false, 1006)]
        public static void ShowWindow()
        {
            window = GetWindow<TVETBHub>(false, "Terrain Blanket Module", true);
        ...
        void OnEnable()
        {
            ...
            searchFolders = AssetDatabase.FindAssets("Terrain Blanket");

            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("Terrain Blanket.pdf"))
                {
                    assetFolder = AssetDatabase.GUIDToAssetPath(searchFolders[i]);
                    assetFolder = assetFolder.Replace("/Terrain Blanket.pdf", "");
                }
            }
            ...
            for (int i = 0; i < searchFolders.Length; i++)
            {
                if (AssetDatabase.GUIDToAssetPath(searchFolders[i]).EndsWith("User.pdf"))
                {
                    ...
                    userFolder += "/The Visual Engine";
                }
            }

            pipelinesPath = assetFolder + "/Core/Pipelines";
            pipelinePath = userFolder + "/Pipeline.asset";
            autorunPath = assetFolder + "/Core/Editor/TVETBHubAutoRun.cs";
            ...
            bannerText = "Terrain Blanket Module";
        ...
        void OnGUI()
        {
            ...
            {
                EditorGUILayout.HelpBox("The current Unity version is not supported! The Visual Engine only supports " + projectData.minimum + " or higher with the " + projectData.pipeline + " Render Pipeline!", MessageType.Error, true);
            ...
            {
                EditorGUILayout.HelpBox("Welcome to the Terrain Blanket Module for The Visual Engine! Choose the render pipeline used in your project to make the shaders compatible with the current pipeline!", MessageType.Info, true);
            ...
            {
                EditorGUILayout.HelpBox("The Terrain Blanket Module is installed for the " + pipelineCurrent + "! Use the additional setting below to restart the render pipeline setup.", MessageType.Info, true);
        ...
        void DrawToolbar()
        {
            ...
            {
                EditorGUIUtility.PingObject(AssetDatabase.LoadAssetAtPath<Object>(assetFolder + "/Demo/Demo Blanket.unity"));
            }
            GUILayout.Space(-1);
            ...
            {
                Application.OpenURL("https://assetstore.unity.com/packages/vfx/shaders/the-vegetation-engine-terrain-blanket-module-254620#reviews");
            }
        ...
        void GetPipeline()
        {
            ...
            if (!pipelineValid)
            {
                ...
                if (version.Contains("2021.3"))
                {
                    pipeline = pipeline + " " + "2021.3+";
                }

                if (version.Contains("2022.3"))
                {
                    pipeline = pipeline + " " + "2022.3+";
                }

                if (version.Contains("6000.0"))
                {
        ...
        void SetDefineSymbols()
        {
            ...
            if (!defineSymbols.Contains("THE_VISUAL_ENGINE_BLANKET"))
            {
                defineSymbols += ";THE_VISUAL_ENGINE_BLANKET;";
